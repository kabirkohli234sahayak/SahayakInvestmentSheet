...
def header_footer_with_logos(canvas, doc):
    ...
    if os.path.exists(LOGO):
        logo_width = 80
        logo_height = 40
        x_center = width / 2.0 - logo_width / 2.0
        y_top = height - (logo_height + 30)
        canvas.drawImage(LOGO, x_center, y_top, width=logo_width, height=logo_height, preserveAspectRatio=True, mask='auto')
    ...
    if factsheet_links.strip():
        ...
        for line in factsheet_links.strip().splitlines():
            if "|" in line:
                ...
                elements.append(Paragraph(f"{label.strip()}: <u><link href='{url.strip()}' color='blue' >{url.strip()}</link></u>", link_container_style))
            else:
                elements.append(Paragraph(line.strip(), styles['Normal']))
...
Sahayak Associates is an AMFI Registered Mutual Fund Distributor only."""
    ...
    disclaimer_elements = [Paragraph("<b>Disclaimer</b>", styles['Heading3'])] + [Paragraph(line.strip(), disclaimer_style) for line in disclaimer_text.strip().splitlines()]
